...
while is_running:
    ...
    if keys[pygame.K_SPACE] and is_hidden:
        is_hidden = False
        animation = 0
        anim_frame = 0
        frame_delay = MAX_FRAME_DELAY
    ...
    if keys[pygame.K_UP] and is_grounded and not is_hidden:
        for door in exits:
            if (x_pos + 4 > door.left and x_pos + P_LENGTH < door.right + 4 \
            and y_pos > door.top and y_pos + P_HEIGHT < door.bottom + 4):
                pygame.mixer.music.stop()
                pygame.mixer.music.load("Audio/AgencyTitle.ogg")
                pygame.mixer.music.play()
                end_screen = pygame.image.load("Images/VictoryScreen.png")
                screen.blit(end_screen, (0, 0, W_LENGTH, W_HEIGHT))
                pygame.display.flip()
